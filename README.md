...
- **CBC Fire Control**: Select verified `cannon_mount`/`compact_cannon_mount` peripherals, calculate geometric line-of-sight aim from coordinate or fresh radar-contact positions, explicitly confirm aiming and firing, and automatically clear the fire signal after a bounded pulse. Ambiguous contacts are rejected. It does not calculate ballistic trajectories; projectile/ammunition behavior must be validated in the target modpack.
...
The 0.2.6 milestone added automation observability and recovery. The 0.3.0–0.4.6 milestones add authenticated encrypted operations, telemetry, bounded defensive response, structured incident coordination, verified CC:CBC cannon-mount telemetry, and operator-confirmed CBC Fire Control. Fire Control uses geometric line-of-sight angles only; it does not claim a ballistic firing solution.
