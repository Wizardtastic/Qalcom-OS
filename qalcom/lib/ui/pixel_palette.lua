-- Extended 256-color palette for the graphics-mode Fluent desktop. Slots 0-15
-- stay the CC defaults in mode 2; we define vibrant Windows 11 tones in slots
-- 16+. Ramp interpolation is pure (no graphics calls) so it is unit-testable.
local PixelPalette = {}
...
function PixelPalette.ramp(stops, steps)
    -- Build `steps` colors by walking a list of hex stops with even spacing.
...
-- Slot allocation (indices into the 256-color palette).
PixelPalette.slots = {
    wallpaperFrom = 16, wallpaperCount = 24,   -- 16..39
    accent = 40, accentDeep = 41, text = 42, textMuted = 43,
    acrylic = 44, acrylicLight = 45,
    card = 46, cardHeader = 47, cardBorder = 48,
    shadowFrom = 49, shadowCount = 4,          -- 49..52
    tileFrom = 53, tileCount = 8,              -- 53..60
    white = 61, startPill = 62,
}

-- A blue "bloom" gradient with a bright azure glow band through the middle.
...
function PixelPalette.plan()
    -- Return a flat list of { slot, hex } to apply, plus named ranges the scene
    -- uses (wallpaper ramp slot list, tile slot list).
...
function PixelPalette.apply(plan)
    -- Apply the plan via term.setPaletteColor (256-color mode). Uses the r,g,b
    -- float form (0..1), which is supported everywhere -- the single 24-bit-hex
    -- form is not accepted by every host and can leave slots unset (black).
    -- No-op without the palette API. Returns the plan for convenience.
