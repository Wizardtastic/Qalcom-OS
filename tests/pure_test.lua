...
test("clamps integer settings", function()
    equal(Pure.clampInteger("9999", 50, 1000, 200), 1000, "upper clamp")
...
test("calculates responsive visual metrics", function()
    equal(Pure.layoutTier(30, 14), "compact", "minimum tier")
    equal(Pure.layoutTier(40, 18), "compact", "narrow everyday tier")
    equal(Pure.layoutTier(80, 24), "standard", "standard tier")
    equal(Pure.layoutTier(120, 40), "wide", "wide tier")
    equal(Pure.layoutTier(204, 76), "command", "command tier")
    local compact = Pure.responsiveMetrics(30, 14)
    equal(compact.outerPadding, 1, "compact outer padding")
    equal(compact.taskbarHeight, 1, "compact taskbar")
    local standard = Pure.responsiveMetrics(80, 24)
    equal(standard.taskbarHeight, 2, "standard taskbar")
    truthy(standard.navigationWidth >= 16, "standard navigation bound")
    local command = Pure.responsiveMetrics(204, 76)
    equal(command.outerPadding, 2, "command outer padding")
    equal(command.sectionGap, 2, "command section gap")
    equal(command.taskbarHeight, 3, "command taskbar")
    equal(command.navigationWidth, 32, "command navigation cap")
    equal(command.inspectorWidth, 36, "command inspector cap")
    truthy(command.width > standard.width and command.height > standard.height, "metrics preserve dimensions")
    equal(Pure.layoutTier(204, 20), "compact", "wide short fallback")
end)
